var gameObject = {
	...
	songsArray: ["BAD", "THRILLER", "BEAT IT", "BILLIE JEAN", "MAN IN THE MIRROR", "BLACK OR WHITE", "ABC", "THE GIRL IS MINE", "THE WAY YOU MAKE ME FEEL", "REMEMBER THE TIME"],
	randomWord: "",
	songLetters:[],
	...
	generateWord: function(){
		...
		this.randomWord = this.songsArray[random_num];
		this.songLetters = this.randomWord.split("");

		//Shows that a randomly chosen song name was converted into an array containing each of its letters.
		console.log(this.randomWord + " " + this.songLetters);
	...
	checkMatch: function(){
		...
		//Loop for the song names length amount of times.
		//If the guessed letter is equal to the the songs letter at a given index, the counter variable counts up one.
		for (var i=0; i < this.songLetters.length; i++){
			if (this.currentLetter == this.songLetters[i]){
				matchCounter++;
			}
		}
	...
	revealsong: function(){
		...
		//For the number of letters in the songs name, fill the displayed guesses with an underscore.
		if (this.correctGuesses.length == 0){
			...
			//For the length of the songs name,
			for (var i=0; i<this.songLetters.length; i++){
				//If the displayed guess is not the same as songLetters at index i,
				if (this.correctGuessesInOrder[i] != this.songLetters[i]){
					//Loop for correctGuesses length number of times,
					for (var j=0; j<this.correctGuesses.length; j++){
						//If the correctGuesses at j is equal to songLetters at i, the displayedGuess becomes the songletter at index i
						if (this.correctGuesses[j] == this.songLetters[i]){
							this.correctGuessesInOrder[i] = this.songLetters[i];
						}
						//Otherwise the displayedGuess at index i (corresponding to the song letter's indexes) becomes an underscore.
						else {
							this.correctGuessesInOrder[i] = "_";
						}
					}
	...
	checkProgress: function(){
		...
		//Loop a number of times equal to the length of the song name. 
		//If a guess is equal to the the song letter at the same index, add 1 to the counter.
		for (var i=0; i<this.songLetters.length; i++){
			if (this.correctGuessesInOrder[i] == this.songLetters[i]){
				counter++;
			}
		...
		//If the counter is the length of the song name, the user has won.
		if (counter == this.songLetters.length){
			alert("You win");
			this.winCount++;
			this.generateWord();
		}
...
document.onkeyup = function(q) {
	...
	//Reveals the song name as it is being guessed.
	gameObject.revealsong();
...
// INSTRUCTIONS: Play a sound or song when the user guesses their word correctly, like in our demo?...Need help in learning how to play the sound for each song. 